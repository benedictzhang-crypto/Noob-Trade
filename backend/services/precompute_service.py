...
class PrecomputeService:
    ...
    def sync_symbols_incremental(self, symbols=None, timeframes=None, window_sizes=None, history_limit=1400):
        selected_symbols = tuple(symbols or self.config["PRECOMPUTE_DEMO_SYMBOLS"])
        results = []

        for symbol in selected_symbols:
            response_data = self._build_incremental_seed_response(symbol, window_sizes, history_limit=history_limit)
            cache_result = self.persistence_service.warm_symbol_cache(
                response_data,
                timeframes=timeframes,
                window_sizes=window_sizes,
            )
            cache_result["mode"] = "incremental"
    ...
    def _build_incremental_seed_response(self, symbol, window_sizes, history_limit=1400):
        lookback_window = max(window_sizes or self.persistence_service.SUPPORTED_WINDOW_SIZES)
        indicators = self.config["DEFAULT_INDICATORS"]

        if not self.market_data_service.market_api.is_configured():
            return build_mock_stock_pattern_analysis(
                symbol=symbol,
                interval=self.config["DEFAULT_INTERVAL"],
                lookback_window=lookback_window,
        ...
        return self.market_data_service._build_live_response(
            ...
            lookback_window=lookback_window,
            indicators=indicators,
            price_limit=max(lookback_window, history_limit),
        )
